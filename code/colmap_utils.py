...
class ColmapReconstruction:
    ...
    def get_image_point3D_ids(self, image_id: int) -> Set[int]:
        if self._image_point3D_ids is None or image_id not in self._image_point3D_ids:
            return set()
        return self._image_point3D_ids[image_id]

    def get_image_point3D_xy(self, image_id: int) -> Dict[int, np.ndarray]:
        if self._image_point3D_xy is None or image_id not in self._image_point3D_xy:
            return {}
        return self._image_point3D_xy[image_id]

    def get_calibration_and_pose(self, image_id: int, target_w: int, target_h: int) -> tuple:
        ...
        K = self.get_camera_calibration_matrix(image_id)
        K_scaled = K.copy()
        K_scaled[0, :] *= scale_x
        K_scaled[1, :] *= scale_y

        pose_4x4 = np.eye(4)
        pose_4x4[:3, :] = self.get_image_cam_from_world(image_id).matrix()

        return K_scaled, pose_4x4, scale_x, scale_y
    ...
    def compute_average_parallax(self, shared_points: Set[int], img1_id: int, img2_id: int, sample_size: int = 100) -> float:
        ...
        return float(np.mean(parallax_angles))

    def find_similar_images_for_image(self, image_id: int, min_points: int = 10, use_folder_filter=False) -> List[int]:
        """
        Finds similar images for a given image based on shared 3D points. If use_folder_filter is True, picks
        images from the same folder as the image_id.
        """
        ...
        if use_folder_filter:
            folder_name = os.path.dirname( self.get_image_name(image_id) )
        ...
        for other_image in self.reconstruction.images.values():
            if other_image.image_id == image_id:
                continue
            if use_folder_filter:
                if os.path.dirname( self.get_image_name(other_image.image_id) ) != folder_name:
                    continue
    ...
    def find_shared_point_list(self, image_id0: int, image_id1: int) -> List[int]:
        ...
        return list(self.get_image_point3D_ids(image_id0) & self.get_image_point3D_ids(image_id1))

    def find_shared_point_list_with_cluster(self, image_id: int, cluster: List[int]) -> List[int]:
        """
        Finds the shared 3D points between an image and a cluster of images.
        """
        if not self.has_image(image_id):
            return []
        self._ensure_image_point_maps()
        ...
        for other_image_id in cluster:
            shared_points.update(self.find_shared_point_list(image_id, other_image_id))
        return list(shared_points)

    def find_image_cluster(self, ref_image_id: int, n_max_images: int, min_points: int = 10, images_to_skip: Set[int] = set[int]()) -> List[int]:
        """
        Finds a cluster of images that are similar to the reference image.
        Args:
            ref_image_id: ID of the reference image
            n_max_images: Maximum number of images to add to the cluster
            min_points: Minimum number of shared 3D points required
            images_to_skip: Images to skip during the search
        Returns:
            List of image IDs in the cluster
        """
        if not self.has_image(ref_image_id):
            return []
        self._ensure_image_point_maps()

        # Check if the current image has any 3D points
        if self._image_point3D_ids is None or ref_image_id not in self._image_point3D_ids:
            return []

        cluster = [ref_image_id]

        if n_max_images == 1:
            return cluster

        # number of images to add at each step
        N_step = 2

        while len(cluster) < n_max_images:
            match_candidates = []
            for other_image in self.reconstruction.images.values():
                if other_image.image_id in cluster:
                    continue
                if other_image.image_id in images_to_skip:
                    continue
                shared_points = self.find_shared_point_list_with_cluster(other_image.image_id, cluster)
                if len(shared_points) >= min_points:
            ...
            match_candidates = match_candidates[:N_step]
            cluster.extend([m.image_id for m in match_candidates])
            if len(cluster) >= n_max_images:
                break

        cluster = cluster[:n_max_images]
    ...
    def compute_robust_bounding_box(self, min_visibility: int = 3, padding_factor: float = 0.1, verbose: bool = True) -> None:
        """
    ...
    def get_visible_3d_points(self, image_id: int, min_track_length: int = 0) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        visible_point_ids = self.get_image_point3D_ids(image_id)

        if visible_point_ids is None:
        ...
        visible_point_xy = self.get_image_point3D_xy(image_id)
        for point_id in visible_point_ids:
            point3d = self.reconstruction.points3D[point_id]
            point2d_xy = visible_point_xy[point_id]
    ...
    def get_camera_resolution(self, image_id: int) -> Tuple[int, int]:
        """Get camera resolution by ID."""
        camera = self.get_image_camera(image_id)
        return camera.width, camera.height

    def trianglate_tracks(self, tracks: List[List[Tuple[int, float, float]]]) -> None:
        """Triangulate tracks using COLMAP."""
        points_3d = []
        for track in tracks:
            image_ids = [image_id for image_id, x, y in track]
            points_2d = np.array([[x, y] for image_id, x, y in track])

            A = []
            for i in range(len(image_ids)):
                P = self.get_camera_projection_matrix(image_ids[i])
                x, y = points_2d[i]
                A.append(x * P[2] - P[0])
                A.append(y * P[2] - P[1])
            A = np.array(A)
            ...
            for image_id in image_ids:
                P = self.get_camera_projection_matrix(image_id)
    ...
    def triangulate_matches(self, iid_i: int, iid_j: int, matches: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
        """Triangulate matches using OpenCV (vectorized).

        Args:
            iid_i: Image ID of the first image
            iid_j: Image ID of the second image
        ...
        P_i = self.get_camera_projection_matrix(iid_i)[:3, :].copy()
        P_j = self.get_camera_projection_matrix(iid_j)[:3, :].copy()
        return geometric_utility.triangulate_matches(P_i, P_j, matches)
...
def find_exact_image_match_from_extrinsics(target_reconstruction: ColmapReconstruction, R: np.ndarray, t: np.ndarray) -> Optional[int]:
    """
    Find the exact match of R,C from target reconstruction.
    """

    min_rotation_angle =  100
    min_translation_diff = 100
    min_image_id = None
    ...
    for image_id in target_reconstruction.get_all_image_ids():
        ...
        if rotation_angle < min_rotation_angle and translation_diff < min_translation_diff:
...
def compute_image_depthmap(reconstruction, image_id, intrinsics, cam_from_world, target_w, target_h, min_track_length=1, verbose=False):
    ...
    try:
        ...
        depth_map, _ = geometric_utility.compute_depthmap(points_3d, intrinsics, cam_from_world, target_w, target_h)
